...
public class CompTeleOp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        //hardware map for Drivetrain - LC 2024
        backleftDrive = hardwareMap.get(DcMotor.class, "BL");
        backrightDrive = hardwareMap.get(DcMotor.class, "BR");
        frontleftDrive = hardwareMap.get(DcMotor.class, "FL");
        frontrightDrive = hardwareMap.get(DcMotor.class, "FR");

        //hardware map for butter gripper tower of terror - LC 2024
        ...
        //Controller instances - LC 12/10/24
        ...
        //this sets the direction the motor will spin for the drive wheels - LC 2024
        frontleftDrive.setDirection(DcMotor.Direction.REVERSE);
        backleftDrive.setDirection(DcMotor.Direction.REVERSE);
        frontrightDrive.setDirection(DcMotor.Direction.FORWARD);
        backrightDrive.setDirection(DcMotor.Direction.FORWARD);

        //sets the directions of the two parallel viper slides in opposite directions so they will move up and down in unison. - LC 2024
        ...
        while (opModeIsActive()) {
            ...
            //sets the power that goes to the viper slides to be reliant on gamepad 2 (operator) left stick when you move it in the y direction - LC 2024
            double vSPower = -gamepad2.left_stick_y;
            leftviperSlide.setPower(vSPower);
            rightviperSlide.setPower(vSPower);
            //sets the power that goes to the joint on the hanger and intake to be reliant on gamepad 2 (operator) right stick when you move it in the y direction - LC 2024
            double jointPower = -gamepad2.right_stick_y;
            //jointPower *= scalingFactor;
            hiJoint.setPower(jointPower);
            // Use the DriveController to handle driving - LC 12/10/24
            driveController.drive(gamepad1);

            /* set's it so that the butter gripper will be right all the way open, left all the way close or prob just chill. hopefully just chill. This is also with a CR Servo so 1 and -1 are max power to the system. you can do half power but DON'T SET THE POWER -1<=X<=1 OR I WILL FIND YOU!!- LC 2024 */
            if (gamepad2.right_bumper) {         // go max open using clawController - LC 12/10/24
                clawController.open();
            } else if (gamepad2.left_bumper) {   // go to closed using clawController - LC 12/10/24
                clawController.close();
            } else {                             // go to no power using clawController - LC 12/10/24
                clawController.stop();
            }

            //set power and control of the servo for intake to the (op) triggers - LC 2024
            if (gamepad2.right_trigger > 0.5){          // intake using intakerollerController - LC 12/10/24
                intakerollerController.intake();
            } else if (gamepad2.left_trigger > 0.5) {   // release using intakerollerController - LC 12/10/24
                intakerollerController.release();
            } else {                                    // stop using intakerollerController - LC 12/10/24
                intakerollerController.stop();
            }

            //set power and control of the motor for extendy to the (op) dpad L and R - LC 2024
            if (gamepad2.dpad_right){               // extend hiExtend arm using hiExtendController - LC 12/10/24
                hiExtendController.extend();
            } else if (gamepad2.dpad_left) {        // retract hiExtend arm using hiExtendController - LC 12/10/24
                hiExtendController.retract();
            } else {                                // stop hiExtend arm using hiExtendController - LC 12/10/24
