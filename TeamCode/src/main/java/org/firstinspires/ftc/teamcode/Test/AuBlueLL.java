...
@Autonomous(name = "AuBlueLL", preselectTeleOp = "CompTeleOp", group = "Test")
public class AuBlueLL  extends LinearOpMode {
    ...
    * */
    ...
    /*public void Plan_B() {
        ...
        while (opModeIsActive() && timer2.seconds() <= 30 && switchToPlanB) {
        ...
        if (timer2.seconds() >= 31 && switchToPlanB) {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addData("Limelight Status", "Pipeline Index: %d", status.getPipelineIndex());

        if (result != null) {
            telemetry.addData("Limelight Target Area (ta)", ta);
            telemetry.addData("Limelight Valid Target?", result.isValid() ? "Yes" : "No");
        } else {
        ...
        // B = 10 - LC 12/30/24
        Trajectory trajectory9 = drive.trajectoryBuilder(trajectory4.end())
                .back(10) // 10
                .build();

        // T = -180, F = 18, SL = 60 - LC 12/30/24
        TrajectorySequence trajectory5 = drive.trajectorySequenceBuilder(trajectory9.end())
                .turn(Math.toRadians(-180)) // 180 deg
                .forward(18) // 18
        ...
        // SL = 10, B = 30, SL = 7, F = 45, B = 50, SL = 7, F = 45 - LC 12/30/24
        TrajectorySequence trajSeq = drive.trajectorySequenceBuilder(trajectory9.end())
                .strafeRight(10) // 10
                .back(30)// 30
                .strafeLeft(7) // 7
                .forward(45) // 45
        ...
        while (opModeIsActive()) {
            ...
            if (ta != 0) {
                telemetry.addData("Limelight Target Area (ta)", currentResult.getTa());
                telemetry.addData("Limelight Valid Target?", currentResult.isValid() ? "Yes" : "No");
                telemetry.addData("I have Butter", "Continuing with Plan A");
                sleep(500);
                // T = -180, F = 18, SL = 60 - LC 12/30/24
                drive.followTrajectorySequence(trajectory5);
            ...
            while (ta == 0 && !switchToPlanB) {  // If no specimen is found, switch to Plan B - LC 12/30/24
